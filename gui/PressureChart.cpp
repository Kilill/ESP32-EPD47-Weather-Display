/**
 * @file PressureChart.cpp
 * @brief Atmospheric pressure chart widget implementation
 * 
 * Renders atmospheric pressure history as line chart with:
 * - Auto-scaling Y-axis around typical pressure range (980-1040 hPa)
 * - hPa unit label above Y-axis
 * - HH:MM time labels on X-axis
 * - Extra space for 4-digit pressure values
 */
...
PressureChart::PressureChart(int x, int y, int width, int height, uint8_t* framebuffer, const GFXfont* font)
    : Chart(x, y, width, height, framebuffer, font) {
    
    // Set typical atmospheric pressure range (980-1040 hPa/mb)
    // Can be auto-scaled based on actual data
    setYRange(980.0f, 1040.0f);
    setAutoScale(true);
}

void PressureChart::updatePressureData(const std::vector<float>& pressures, const std::vector<uint32_t>& timestamps) {
    std::vector<DataPoint> points;
    
    size_t count = std::min(pressures.size(), timestamps.size());
    for (size_t i = 0; i < count; i++) {
        DataPoint point;
        point.value = pressures[i];
...
void PressureChart::drawTitle() {
    const char* title = "Atmospheric Pressure";
    
    int32_t title_x = plot_x + plot_width / 2 - 80; // Center approximately
    int32_t title_y = plot_y - 10;
    
    write_string(font, title, &title_x, &title_y, fb);
}

void PressureChart::drawYLabels() {
    char label[16];
    
    // Draw unit label above the top tick
    int32_t unit_x = plot_x - 50;
    int32_t unit_y = plot_y - 15;
    write_string(&smallFont, "hPa", &unit_x, &unit_y, fb);
...
void PressureChart::drawXLabels() {
